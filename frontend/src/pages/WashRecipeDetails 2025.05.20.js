  ...
  useEffect(() => {
    const fetchWashRecipeDetails = async () => {
      try {
        ...
        const { washRecipe, steps, chemicalsByStep, processes, combinedItems } =
          response;
...
// 1. Create a proper custom node component
  ...
  const nodes = [
    {
      id: "start",
      type: "input",
      data: { label: "Start", tooltip: "Start of the wash process",},
      position: { x: 0, y: 0 },
    ...
    ...combinedItems?.map((item, index) => ({
      ...
      data: {
        ...
        tooltip: item?.stepId
          ? `Time: ${item?.time} min, Temp: ${item?.temp}°C, Liters: ${item?.liters}`
          : `Process Type: ${item?.laundryProcessId?.type}, Remark: ${
    ...
    })),
    {
      id: "end",
      type: "output",
      data: { label: "End", tooltip: "End of the wash process", },
    ...
    <div className="p-6 bg-gray-50 min-h-screen">
          ...
          <div className="mb-6 p-6 rounded bg-gray-50 border border-gray-200 shadow-sm">
            ...
            <table className="min-w-full bg-white border border-gray-200 table-auto rounded-lg shadow-lg">
              <thead>
                ...
                >
                  <th className="py-2 px-4 border-b border-gray-200 cursor-pointer border p-2">
                    Wash Code
                  </th>
                  <th className="py-2 px-4 border-b border-gray-200 cursor-pointer border p-2">
                    Wash Type
              ...
              <tbody>
                <td className="py-2 px-4 border text-center">
                  {washRecipe.washCode || "N/A"}
                </td>
                <td className="py-2 px-4 border text-center">
                  <span
                    className={`px-2 py-1 rounded text-sm ${
                      washTypeColors[washRecipe.washType] ||
                ...
                <td className="py-2 px-4 border text-center">
                  {new Date(washRecipe.date).toLocaleDateString() || "N/A"}
                </td>
              </tbody>
            </table>
          </div>
            ...
            <table className="min-w-full bg-white border border-gray-200 table-auto rounded-lg shadow-lg">
              <thead>
                <tr
                  ...
                  style={{ backgroundColor: hoverColor }}
                >
                  <th className="border-b-2 border-gray-200 cursor-pointer border p-2">
                    Order No
                  </th>
                  <th className="border-b-2 border-gray-200 cursor-pointer border p-2">
                    style
              ...
              <tbody>
                <td className="p-2 border text-center">
                  {washRecipe.orderId.orderNo}
                </td>
                <td className="p-2 border text-center">
                  {washRecipe.orderId?.style?.name || "N/A"}
                </td>
                <td className="p-2 border text-center">
                  {washRecipe.orderId?.articleNo || "N/A"}
                </td>
                {/* <td className="py-2 px-4">{washRecipe.orderId.fabricSupplier}</td> */}
                <td className="p-2 border text-center">
                  {washRecipe.orderId.keyNo}
                </td>
                {/* <td className="py-2 px-4">{washRecipe.orderId.qty}</td> */}
                <td className="p-2 border text-center">
                  {new Date(
                    washRecipe.orderId.orderDate
      ...
      <div className="mb-6">
        ...
        <div style={{ height: "500px" }} className="rounded-lg shadow-lg p-4">
          ...
          {/* Render Tooltip */}
          {/* Single Tooltip component for all nodes */}
          <Tooltip 
            id="reactflow-tooltip"
            place="top"
            effect="solid"
            style={{ zIndex: 100 }}
          />
            ...
            {processes.map((process) => (
              ...
              >
                ...
                <p>
                  ...
                  {process.remark || "No remarks provided"}
                </p>
              </div>
            ))}
          </div>
                        ...
                        {item.chemicals.map((chemical, idx) => (
                          <li key={idx}>
                            ...
                            <span className="italic text-gray-500">
                              (Supplier:{" "}
                              {chemical.chemicalItemId.supplier || "N/A"})
                            </span>
                          </li>
                        ))}
                      </ul>
