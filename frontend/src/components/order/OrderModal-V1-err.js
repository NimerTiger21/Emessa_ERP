...
const OrderModal = ({
  ...
  useEffect(() => {
    if (
      ...
      console.log("selectedStyle BEFORE FormData: ", selectedStyle);
      // Set available style numbers FIRST
      const newAvailableStyleNumbers = selectedStyle?.styleNo || [];
      console.log('Style numbers from object:', newAvailableStyleNumbers); // Debug log
      setAvailableStyleNumbers(newAvailableStyleNumbers);
      const mappedEditOrder = {
        ...
        // Check for missing/invalid deliveryDate
        ...
        deliveryDate: editOrder.deliveryDate
          ...
          : null,
        // Ensure styleNo is carried over from the original editOrder
        // styleNo: editOrder.styleNo || selectedStyle?.styleNo?.[0] || "",

        // Keep the original styleNo from editOrder
        // styleNo: editOrder.styleNo || "",

        // Use the editOrder.styleNo if it exists in the available numbers, otherwise use first available
        styleNo:
          editOrder.styleNo &&
          newAvailableStyleNumbers.includes(editOrder.styleNo)
            ? editOrder.styleNo
            : newAvailableStyleNumbers[0] || "",
      };
      ...
      // Set available style numbers based on selected style
      if (selectedStyle) {
        setAvailableStyleNumbers(selectedStyle.styleNo || []);

        // Validate that the stored styleNo exists in the style's styleNo array
        if (
          editOrder.styleNo &&
          !selectedStyle.styleNo.includes(editOrder.styleNo)
        ) {
          console.warn(
            `Style number ${editOrder.styleNo} not found in style's options`
          );
        }
      } else {
      ...
      console.log("New available numbers: Before setFormData - available:", newAvailableStyleNumbers);
      console.log("Original styleNo:", editOrder.styleNo);
      console.log("Final styleNo:", mappedEditOrder.styleNo);
      setFormData(mappedEditOrder);
      console.log("Selected Style:", selectedStyle);
      console.log("Available Style Numbers:", availableStyleNumbers);
      console.log("Current StyleNo in form:", formData.styleNo);
      console.log("formData.style type:", typeof formData.style); // should log "string"

      console.log("formData.style:", formData.style);
console.log("formData.style._id:", formData.style?._id);
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                >
                  ...
                  {availableStyleNumbers.map((styleNo) => (
