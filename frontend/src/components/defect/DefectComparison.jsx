...
const DefectComparison = () => {
  ...
  const fetchComparisonData = async () => {
    try {
      ...
      const correlationMatrix = [
        {
          category: "Fabric",
          fabric: 100,
          style: calculateCorrelation(fabricData, styleData),
          composition: calculateCorrelation(fabricData, compositionData),
        },
        {
          category: "Style",
          fabric: calculateCorrelation(styleData, fabricData),
          style: 100,
          composition: calculateCorrelation(styleData, compositionData),
        },
        {
          category: "Composition",
          fabric: calculateCorrelation(compositionData, fabricData),
          style: calculateCorrelation(compositionData, styleData),
          composition: 100,
        },
      ];
      ...
      const performanceMetrics = {
        ...
        avgCompositionDefectRate:
          compositionData.reduce(
            (sum, item) => sum + parseFloat(item.percentage),
            0
  ...
  const calculateCorrelation = (data1, data2) => {
    ...
    const numerator = subset1.reduce(
      (sum, val, i) => sum + (val - mean1) * (subset2[i] - mean2),
      0
    );
    const denominator1 = Math.sqrt(
      subset1.reduce((sum, val) => sum + Math.pow(val - mean1, 2), 0)
    );
    const denominator2 = Math.sqrt(
      subset2.reduce((sum, val) => sum + Math.pow(val - mean2, 2), 0)
    );

    return denominator1 && denominator2
      ? Math.abs((numerator / (denominator1 * denominator2)) * 100)
      : 0;
  };
    ...
    <div className="space-y-6">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-gray-800">
                {comparisonData.performanceMetrics.avgFabricDefectRate?.toFixed(
                  1
                ) || 0}
                %
              </p>
        ...
        <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-100">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-gray-800">
                {comparisonData.performanceMetrics.avgStyleDefectRate?.toFixed(
                  1
                ) || 0}
                %
              </p>
          ...
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-500">
                Avg Composition Defect Rate
              </p>
              <p className="text-2xl font-bold text-gray-800">
                {comparisonData.performanceMetrics.avgCompositionDefectRate?.toFixed(
                  1
                ) || 0}
                %
              </p>
              <div className="flex items-center mt-1">
                ...
                <span className="text-sm text-green-600">
                  Total Defects:{" "}
                  {comparisonData.performanceMetrics.totalDefects}
                </span>
              </div>
            </div>
      ...
      <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-100">
        <h3 className="text-lg font-medium text-gray-800 mb-4">
          Fabric vs Style Defect Rate Comparison
        </h3>
      ...
      <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-100">
        ...
        <p className="text-sm text-gray-600 mb-4">
          Shows the correlation strength between different defect categories.
                ...
                <tr key={index}>
                  ...
                  <td className="px-4 py-2 text-center">
                    <div className="flex items-center justify-center">
                      <div
                        className={`w-3 h-3 rounded-full mr-2 ${getCorrelationColor(
                          row.fabric
                        )}`}
                      ></div>
                      <span className="text-sm">{row.fabric.toFixed(0)}%</span>
                      <span className="text-xs text-gray-500 ml-1">
                        ({getCorrelationText(row.fabric)})
                      </span>
                    </div>
                  </td>
                  <td className="px-4 py-2 text-center">
                  ...
                  <td className="px-4 py-2 text-center">
                    <div className="flex items-center justify-center">
                      ...
                      <span className="text-sm">
                        {row.composition.toFixed(0)}%
                      </span>
                      <span className="text-xs text-gray-500 ml-1">
                        ({getCorrelationText(row.composition)})
                      </span>
                    </div>
                  </td>
                </tr>
      ...
      <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-100">
        <h3 className="text-lg font-medium text-gray-800 mb-4">
          Defect Rate Scatter Analysis
        </h3>
        <p className="text-sm text-gray-600 mb-4">
          Relationship between fabric defect rates and style defect rates.
          Points closer to the diagonal line indicate higher correlation.
        </p>
          ...
          <ResponsiveContainer width="100%" height="100%">
            <ScatterChart data={comparisonData.fabricVsStyle} margin={{ top: 20, right: 20, bottom: 40, left: 40 }}>
              ...
              <Tooltip
                formatter={(value, name) => [
                  `${value}`,
                  name === "Fabric Defect Rate" ? "Fabric Rate" : "Style Rate",
                ]}
                labelFormatter={() => ""}
              />
              <Scatter
      ...
      <div className="bg-gradient-to-r from-blue-50 to-indigo-50 p-6 rounded-lg border border-blue-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <ul className="text-sm text-gray-600 space-y-1">
              <li>
                • Focus quality control on high-correlation fabric-style
                combinations
              </li>
              <li>
                • Implement targeted inspections for top-performing defect
                categories
              </li>
              <li>
                • Review supplier quality agreements for problematic materials
              </li>
            </ul>
          </div>
        </div>
