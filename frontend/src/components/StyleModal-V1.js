...
const StyleModal = ({ isOpen, closeModal, editStyle, refreshStyleList }) => {
  ...
  const [style, setStyle] = useState({ name: "", styleNo: [""] });
  ...
  const handleStyleNoChange = (index, value) => {
    const updated = [...style.styleNo];
    updated[index] = value;
    setStyle((prev) => ({ ...prev, styleNo: updated }));
  };

  const addStyleNo = () => {
    setStyle((prev) => ({ ...prev, styleNo: [...prev.styleNo, ""] }));
  };

  const removeStyleNo = (index) => {
    const updated = style.styleNo.filter((_, i) => i !== index);
    setStyle((prev) => ({ ...prev, styleNo: updated }));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editStyle) {
        ...
        await createStyle(combinedData);
