      ...
      <div className="bg-white p-3 border border-gray-200 shadow-md rounded-md">
        ...
        {payload.map((entry, index) => (
          <p key={index} style={{ color: entry.color }} className="text-sm">
            ...
            {entry.name === "percentage" || entry.name === "defectRatio" ? "%" : ""}
          </p>
        ))}
      </div>
    );
    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div className="bg-indigo-50 rounded-lg p-4">
          ...
          <p className="text-3xl font-bold text-indigo-800">
            {data.summary?.totalDefects || 0}
          </p>
          <p className="text-indigo-600 text-sm mt-1">
            {data.summary?.totalWashRecipeDefects || 0} with defects
          </p>
        </div>
        <div className="bg-amber-50 rounded-lg p-4">
          <h3 className="text-amber-900 font-medium mb-1">Avg. Defect Ratio</h3>
          <p className="text-3xl font-bold text-amber-800">
            {data.summary?.washRecipeDefectRatio || 0}%
          </p>
        ...
        <div className="bg-emerald-50 rounded-lg p-4">
          ...
          <p className="text-3xl font-bold text-emerald-800">
            {data.topDefectiveRecipes && data.topDefectiveRecipes[0]
              ? `${parseFloat(data.topDefectiveRecipes[0].defectRatio).toFixed(
                  2
                )}%`
              : "0%"}
          </p>
      ...
      <div className="border border-gray-200 rounded-lg p-4 mb-6 bg-white">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div>
                <p className="text-sm font-medium text-gray-500 mb-2">
                  Defect Rate by Wash Type
                </p>
                <ResponsiveContainer width="100%" height={chartHeight}>
                  <BarChart data={data.byWashType || []} barSize={30}>
              ...
              <div>
                <p className="text-sm font-medium text-gray-500 mb-2">
                  Defect Rate by Process Type
                </p>
                <ResponsiveContainer width="100%" height={chartHeight}>
                  <BarChart data={data.byProcess || []} barSize={40}>
                    <CartesianGrid strokeDasharray="3 3" vertical={false} />
                    <XAxis dataKey="name" />
                    <YAxis yAxisId="left" orientation="left" stroke="#4f46e5" />
                    <YAxis
        ...
        {activeChart === "washType" && (
          <div>
            ...
            <ResponsiveContainer width="100%" height={400}>
              <RadarChart outerRadius={150} data={data.byWashType || []}>
                <PolarGrid />
        ...
        {activeChart === "processType" && (
          <div>
            <h3 className="text-lg font-medium text-gray-800 mb-4">
              Defects by Process Type
            </h3>
            <ResponsiveContainer width="100%" height={400}>
              <BarChart data={data.byProcess || []} layout="vertical">
                <CartesianGrid
                  strokeDasharray="3 3"
                  horizontal={true}
                  vertical={false}
                ...
                <Tooltip content={<CustomTooltip />} />
                <Legend />
                <Bar dataKey="count" name="Total Defects" fill="#4f46e5">
                  {(data.byProcess || []).map((entry, index) => (
                    <Cell
                      key={`cell-${index}`}
                      fill={
                        PROCESS_COLORS[entry.name] ||
        ...
        {activeChart === "scatter" && (
          <div>
            <h3 className="text-lg font-medium text-gray-800 mb-4">
              Order Quantity vs Defect Ratio
            </h3>
            <ResponsiveContainer width="100%" height={400}>
              <ScatterChart
              ...
              >
                ...
                {formatScatterData().map((washType, index) => {
                  ...
                  const color =
                    WASH_TYPE_COLORS[type] || COLORS[index % COLORS.length];
                    ...
                    >
                      {formatComparisonData().top.map((entry, index) => (
                        <Cell
                          key={`cell-${index}`}
                          fill={
                            WASH_TYPE_COLORS[entry.washType] ||
                            COLORS[index % COLORS.length]
                          }
                        />
                      ))}
