...
const StepsTable = ({ steps, setSteps, setWorkspaceItems, workspaceItems }) => {
  ...
  const handleChemicalAdded = (stepId, newChemical, stepInstanceId) => {
    ...
    const stepIndex = steps.findIndex(
      (step) => step.stepId === stepId && step.id === stepInstanceId
    );
    if (stepIndex === -1) {
  ...
  const handleAddStep = () => {
    ...
    // ❌ This only checks steps array.
    // ✅ Replace with:

    const existingSequences = [...steps, ...workspaceItems].map(
      (item) => item.sequence
    );
    ...
    // For workspace items, we need to handle sequencing carefully
    setWorkspaceItems((prevItems) => {
      // Find the highest sequence number currently in use
      const highestSequence =
        prevItems.length > 0
          ? Math.max(...prevItems.map((item) => item.sequence || 0))
          : 0;
      ...
      return [...prevItems, stepWithSequence];
    });
    ...
    setWorkspaceItems((prevItems) => {
      ...
      const filteredItems = prevItems.filter(
        (item) =>
          !(
            item.type === "step" &&
            item.sequence === stepToRemove.sequence &&
            item.stepId === stepToRemove.stepId
          )
      );
            ...
            <tr key={index} className="hover:bg-blue-50 transition-colors">
              <td className="border p-2 text-center">
                <select
                  ...
                  onChange={(e) => {
                    ...
                    handleStepChange(index, "stepId", selectedStepId);
                  }}
              ...
              <td className="border p-2 text-center">
                <input
                  ...
                  value={step.temp}
                  onChange={(e) =>
                    handleStepChange(index, "temp", e.target.value)
                  }
                  className="border p-2 rounded focus:ring-blue-500 focus:border-blue-500"
                  placeholder="°C"
                />
              ...
              <td className="border p-2 text-center">
                <input
                  ...
                  value={step.liters}
                  onChange={(e) =>
                    handleStepChange(index, "liters", e.target.value)
                  }
                  className="border p-2 rounded focus:ring-blue-500 focus:border-blue-500"
                  placeholder="Liters"
                  min="0"
